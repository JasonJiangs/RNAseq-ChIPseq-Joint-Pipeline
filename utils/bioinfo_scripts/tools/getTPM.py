...
parser.add_option('-i', '--input', '--in', default='.',
...
parser.add_option('-t', default='transcript_tpm_matrix.csv',
                  help="where to output the transcript tpm matrix [default: %default]")
...
if (os.path.isfile(opts.input)):
    ...
    #####
    samples = [(i, glob.iglob(os.path.join(opts.input, i, "*.gtf")).next()) for i in next(os.walk(opts.input))[1] if
               re.search(opts.pattern, i)]
        ...
        for i, v in enumerate(split):
            if is_transcript(v):
                ...
                if not RE_STRING.match(g_id):
                    badGenes.append([v[0], v[6], t_id, g_id, min(int(v[3]), int(v[4])), max(int(v[3]), int(
                        v[4]))])  # chromosome, strand, cluster/transcript id, start, end
                    j = i + 1
                    while j < len(split) and split[j][2] == "exon":
                        badGenes[len(badGenes) - 1].append(
                            (min(int(split[j][3]), int(split[j][4])), max(int(split[j][3]), int(split[j][4]))))
                        j += 1
            ...
            if l.startswith('#'):
                if lno == 1:
                    ...
                    if gf:
                        ...
                        if guidesFile:
                            if gfile != guidesFile:
                                print("Warning: sample file %s generated with a different -G file (%s) than the first "
                                      "sample (%s)" % (s[1], gfile, guidesFile))
                        else:
                            guidesFile = gfile
                    else:
